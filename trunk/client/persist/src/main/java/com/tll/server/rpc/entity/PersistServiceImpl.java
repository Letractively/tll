...
/**
 * PersistServiceImpl - Persist/crud functionality for model data.
 * @author jpk
 * @param <E>
 */
public abstract class PersistServiceImpl<E extends IEntity> implements IPersistServiceImpl<E> {
	...
	/**
	 * Generic delegate method for loading an entity data by name.
	 * @param entityClass
	 * @param svc
	 * @param name
	 * @return the loaded entity
	 * @throws UnsupportedOperationException when the entity type does not support
	 *         the name property
	 */
	protected abstract E loadByName(Class<E> entityClass, IEntityService<E> svc, String name)
	 ...
	 */
	protected abstract void handleLoadOptions(PersistContext context, E e, EntityOptions options,
	 ...
	 * @throws SystemError When any error occurrs.
	 */
	protected abstract void handlePersistOptions(PersistContext context, E e, EntityOptions options) throws SystemError;
	...
	protected E coreLoad(final PersistContext context, final EntityLoadRequest request, final ModelPayload payload) {

		// core entity loading
		final Class<E> entityClass = (Class<E>) context.getEntityTypeResolver().resolveEntityClass(request.getEntityType());
		...
		if(request.isLoadByName()) {
			...
			try {
				return loadByName(entityClass, svc, name);
			}
			catch(final UnsupportedOperationException e) {
				throw new SystemError("Can't load entity type: " + request.getRef().getEntityType().descriptor() + " by name.");
			}
		}

		else if(request.isLoadByBusinessKey()) {
	...
	public final void load(final PersistContext context, final EntityLoadRequest request, final ModelPayload payload) {
		try {
	...
	@SuppressWarnings("unchecked")
	public final void persist(final PersistContext context, final PersistRequest request,
			final ModelPayload payload) {
		Class<E> entityClass = null;
		try {
			// core persist
			entityClass = (Class<E>) context.getEntityTypeResolver().resolveEntityClass(request.getEntityType());
	...
	@SuppressWarnings("unchecked")
	public final void purge(final PersistContext context, final PurgeRequest request, final ModelPayload payload) {
		try {
			final Class<E> entityClass =
				(Class<E>) context.getEntityTypeResolver().resolveEntityClass(request.getEntityType());
			final IEntityService<E> svc = context.getEntityServiceFactory().instanceByEntityType(entityClass);
	 ...
	 *         unsupported.
	 */
	protected abstract void handleSearchTranslation(PersistContext context, ISearch search, ICriteria<E> criteria)
	throws IllegalArgumentException;

	@SuppressWarnings("unchecked")
	public final ICriteria<E> translate(final PersistContext context, final ISearch search)
	throws IllegalArgumentException {
		final CriteriaType criteriaType = search.getCriteriaType();
		final Class<E> entityClass = (Class<E>) context.getEntityTypeResolver().resolveEntityClass(search.getEntityType());
		Criteria<E> criteria;
